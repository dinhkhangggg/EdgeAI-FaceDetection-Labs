...
if len(indices) > 0:
    for i in indices.flatten():
        ...
        pad = int(6 * scale_ui)
        cv2.rectangle(img, (x, y - th - pad), (x + tw + pad, y), color, -1)
        cv2.putText(img, label, (x + int(pad/2), y - int(pad/2)), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255), font_thickness, cv2.LINE_AA)
